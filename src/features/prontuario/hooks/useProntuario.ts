...
import { cleanDigits, maskCPF, maskRG, maskPhone, maskCEP } from '@shared/utils/masks';
import { isValidEmail, isValidCPF, isValidDate, sanitizeText, formatEmail } from '@shared/utils/validators';
import type { PacienteDetalhe, PacienteFormData, NotificationState } from '@features/prontuario/types';
...
export function useProntuario() {
    ...
    const handleUpdateArrayField = (field: 'telefones' | 'enderecos' | 'familiares', index: number, subField: string, value: any) => {
        ...
        if (field === 'enderecos' && subField === 'cep') {
                ...
                try {
                    fetch(`https://viacep.com.br/ws/${cleanCep}/json/`)
                        ...
                        .then(data => {
                            if (!data.erro) {
                                setFormDataPaciente(prev => {
                                    const newArray = [...(prev.enderecos || [])];
                                    if (newArray[index]) {
                                        newArray[index] = {
                                            ...newArray[index],
                                            endereco: data.logradouro || newArray[index].endereco || '',
                                            bairro: data.bairro || newArray[index].bairro || '',
                                            cidade: data.localidade || newArray[index].cidade || '',
                                            estado: data.uf || newArray[index].estado || ''
                                        };
                                    }
                                    return { ...prev, enderecos: newArray };
                                });
                                showToast('Endereço autocompletado do ViaCEP.', 'success');
                            }
                        })
                        .catch(err => console.error("Erro ViaCEP:", err));
                } catch (err) {
                    console.error('Erro geral ao buscar CEP:', err);
                }
            }
        }
    };
